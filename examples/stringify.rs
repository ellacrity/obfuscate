//! This example demonstrates various ways to obfuscate the string literals in more
//! complex scenarios. The example presented here uses an enum because it can be
//! challenging to return a string representation of it.

use std::fmt;

use obfuscate::{obfstr, obfstring, position};
...
impl Example {
    ...
    // Use a callback to keep the string slice allocated on the stack but this gets
    // annoying in more complex scenarios.
    ...
    // Use a buffer to hold the deobfuscated string. Panics if the buffer is too
    // small.
