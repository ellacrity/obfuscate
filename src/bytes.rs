...
///
/// The purpose of the obfuscation is to make it difficult to discover the original
/// strings with automated analysis. String obfuscation is not intended to hinder a
/// dedicated reverse engineer from discovering the original string. This should not
/// be used to hide secrets in client binaries and the author disclaims any
/// responsibility for any damages resulting from ignoring this warning.
///
/// The `obfstr!` macro returns the deobfuscated string as a temporary `&str`
/// value and must be consumed in the same statement it was used:
///
/// ```
/// use obfuscate::obfstr as s;
///
/// const HELLO_WORLD: &str = "Hello 🌍";
/// assert_eq!(s!(HELLO_WORLD), HELLO_WORLD);
/// ```
///
/// Different syntax forms are supported to reuse the obfuscated strings in outer
/// scopes:
///
/// ```
/// use obfuscate::obfstr as s;
...
macro_rules! __obfbytes {
    ($s:expr) => {{
        ...
        const _OBFBYTES_KEYSTREAM: [u8; _OBFBYTES_LEN] =
            $crate::bytes::keystream::<_OBFBYTES_LEN>($crate::random!(
                u32,
                "key",
                stringify!($s)
            ));
        static _OBFBYTES_SDATA: [u8; _OBFBYTES_LEN] =
            $crate::bytes::obfuscate::<_OBFBYTES_LEN>(
                _OBFBYTES_STRING,
                &_OBFBYTES_KEYSTREAM,
            );
...
#[inline(always)]
pub const fn obfuscate<const LEN: usize>(source: &[u8], key: &[u8; LEN]) -> [u8; LEN] {
    if source.len() != LEN {
...
pub fn deobfuscate<const LEN: usize>(s: &[u8; LEN], k: &[u8; LEN]) -> [u8; LEN] {
    ...
    // Use `read_volatile` to avoid constant folding a specific read and optimize the
    // rest Volatile reads of any size larger than 8 bytes appears to cause a bunch
    // of one byte reads Hand optimize in chunks of 8 and 4 bytes to avoid this
    unsafe {
        let src = s.as_ptr();
...
pub fn equals<const LEN: usize>(s: &[u8; LEN], k: &[u8; LEN], other: &[u8]) -> bool {
    ...
    unsafe {
        ...
        while i < LEN & !3 {
            let ct = read_volatile(src.offset(i as isize) as *const [u8; 4]);
            let tmp = u32::from_ne_bytes([ct[0], ct[1], ct[2], ct[3]])
                ^ u32::from_ne_bytes([k[i], k[i + 1], k[i + 2], k[i + 3]]);
            let other = u32::from_ne_bytes([
                other[i],
                other[i + 1],
                other[i + 2],
                other[i + 3],
        ...
        match LEN % 4 {
            ...
            2 => {
                let ct = read_volatile(src.offset(i as isize) as *const [u8; 2]);
                u16::from_ne_bytes([ct[0], ct[1]])
                    ^ u16::from_ne_bytes([k[i], k[i + 1]])
                    == u16::from_ne_bytes([other[i], other[i + 1]])
            }
            3 => {
                let ct = read_volatile(src.offset(i as isize) as *const [u8; 3]);
                u32::from_ne_bytes([ct[0], ct[1], ct[2], 0])
                    ^ u32::from_ne_bytes([k[i], k[i + 1], k[i + 2], 0])
                    == u32::from_ne_bytes([other[i], other[i + 1], other[i + 2], 0])
            }
            _ => true,
        }
    }
