...
macro_rules! obfwide_impl {
    ($s:expr) => {{
        ...
        const _OBFWIDE_KEYSTREAM: [u16; _OBFWIDE_LEN] =
            $crate::words::keystream::<_OBFWIDE_LEN>($crate::random!(
                u32,
                "key",
                stringify!($s)
            ));
        static _OBFWIDE_SDATA: [u16; _OBFWIDE_LEN] =
            $crate::words::obfuscate::<_OBFWIDE_LEN>(
                _OBFWIDE_STRING,
                &_OBFWIDE_KEYSTREAM,
            );
        $crate::words::ddeobfuscate::<_OBFWIDE_LEN>(
            $crate::xref::xref::<
                _,
...
pub const fn keystream<const LEN: usize>(key: u32) -> [u16; LEN] {
    ...
    while i < LEN & !1 {
        ...
        keys[i] = u16::from_ne_bytes([kb[0], kb[1]]);
...
#[inline(always)]
pub fn ddeobfuscate<const LEN: usize>(s: &[u16; LEN], k: &[u16; LEN]) -> [u16; LEN] {
    ...
    unsafe {
        ...
        while i < LEN & !3 {
            ...
            let tmp =
                [ct[0] ^ k[i], ct[1] ^ k[i + 1], ct[2] ^ k[i + 2], ct[3] ^ k[i + 3]];
...
fn test_remaining_bytes() {
    ...
    fn test<const LEN: usize>(key: u32) {
        ...
        let buffer = ddeobfuscate::<LEN>(&data, &keys);
