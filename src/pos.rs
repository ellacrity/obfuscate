...
///
/// ```
/// assert_eq!(obfuscate::position!("haystack", "st"), 3..5);
/// # assert_eq!(obfuscate::position!("haystack", "haystack"), 0..8);
/// # assert_eq!(obfuscate::position!("haystack", "ck"), 6..8);
/// ```
///
/// Use this API when pooling strings in a single obfuscate:
///
/// ```
/// const POOL: &str = concat!("Foo", "Bar", "Baz");
///
/// obfuscate::obfstr! { let pool = POOL; }
///
/// // Later, read strings from the pool
/// let data = &pool[obfuscate::position!(POOL, "Foo")];
/// let bar = &pool[obfuscate::position!(POOL, "Bar")];
/// let baz = &pool[obfuscate::position!(POOL, "Baz")];
/// ```
#[macro_export]
macro_rules! position {
    ($haystack:expr, $needle:expr) => {{
        const _POSITION_RANGE: ::core::ops::Range<usize> =
            $crate::position($haystack, $needle);
        _POSITION_RANGE
    }};
}
...
///
/// ```
/// const POSITION: std::ops::Range<usize> = obfuscate::position("haystack", "st");
